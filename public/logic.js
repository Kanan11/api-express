...
fetch('http://localhost:3012/artists/').then(function (response) {
    if (response.ok) {
...
saveArtistBtn.onclick = async function () {
    ...
    const status = await makeRequest("http://localhost:3012/artists", "POST", { name: name })
    console.log(status)
...
getArtists.onclick = async function () {
    ...
    const visaArtists = await makeRequest("http://localhost:3012/artists", "GET")
    console.log(visaArtists)

    document.getElementById('artistsDiv').innerHTML = "<h3>Här kan se Artist Listan</h3>";
...
updateArtistBtn.onclick = async function () {
    ...
    let response = await makeRequest(`http://localhost:3012/artists/${artistId}`, 'PUT', {  name: discription }); // Här jag skickar id och name
...
deleteArtistBtn.onclick = async function () {
    ...
    fetch('http://localhost:3012/artists/' + name, {
        method: 'DELETE',
    })
